...
test('playwright backend forwards mouse input to the remote page @playwright', async ({ page }) => {
    ...
    await expect(browserViewShell).toHaveScreenshot('portal-mouse-playwright.png', {
        animations: 'disabled',
        caret: 'hide',
        scale: 'css',
    });
...
test('playwright backend forwards keyboard input to the remote page @playwright', async ({ page }) => {
    const browserViewShell = page.getByTestId('browser-view-shell');
    ...
    await expect(browserViewShell).toHaveScreenshot('portal-keyboard-playwright.png', {
        animations: 'disabled',
        caret: 'hide',
        scale: 'css',
    });
