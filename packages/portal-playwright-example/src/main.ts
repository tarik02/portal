import { createPlaywrightExampleServer, DEFAULT_PORT } from './server';
...
const main = async () => {
    const server = await createPlaywrightExampleServer({
        host: readHost(process.env.HOST),
        port: readPort(process.env.PORT),
    });

    console.log(`playwright example ready: ${toExampleHttpUrl(server.url)}`);
